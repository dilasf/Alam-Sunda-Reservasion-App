<x-admin-layout>
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <nav>
                <ol class="flex items-center gap-2">
                    <li>
                        {{-- Admin / --}}
                        <a class="font-medium" href="{{ route('admin.meja.index') }}">Meja /</a>
                    </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <form action="{{ route('admin.meja.update', $meja->idMeja) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="flex flex-col gap-5.5 p-6.5">
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlahPengunjung" id="jumlahPengunjung" placeholder="Kapasitas"
                            class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                            value="{{ old('jumlahPengunjung', $meja->jumlahPengunjung) }}" required />
                        ...
                        <div class="relative z-20 bg-white dark:bg-form-input">
                            <select name="status" required
                                ...
                                @foreach ($statusOptions as $value => $label)
                                    <option value="{{ $value }}" {{ $meja->status == $value ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="lokasi" id="lokasi" placeholder="Lokasi"
                            class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                            value="{{ old('lokasi', $meja->lokasi) }}" required />
                        @error('lokasi')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
